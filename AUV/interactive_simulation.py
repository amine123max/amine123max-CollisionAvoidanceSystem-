...
"""
交互式模拟 - 先生成地图和静态障碍物，然后生成ROV并通过键盘控制移动，让AUV自主避障
"""
...
try:
    ...
    print("错误: 未找到模拟场景地图模块，请确保simulation_map.py文件存在")
    sys.exit(1)
...
class InteractiveSimulation:
    ...
    def __init__(self, map_width=500, map_height=500, 
                 num_obstacles=30, dt=0.1, seed=None, save_dir='results'):
        """
        初始化交互式模拟
        ...
        pygame.display.set_caption('AUV与ROV交互式模拟')
    ...
    def run(self):
        ...
        print("交互式模拟已启动")
        print("使用方向键控制ROV运动，AUV将自主避障")
        print("按ESC退出，按空格暂停/继续，按H显示/隐藏帮助")
    ...
    def _update(self):
        ...
        # 检查ROV是否与障碍物碰撞
        if self.sim_map.check_collision(self.rov_state[0], self.rov_state[1], radius=3.0):
            print(f"警告: 时间 {self.current_time:.1f}s - ROV与障碍物碰撞！")
            # 碰撞后ROV速度减半并反向（模拟碰撞效果）
            self.rov_state[2] *= -0.5
            self.rov_state[3] *= -0.5
        ...
        # 检查AUV是否与障碍物碰撞
        if self.sim_map.check_collision(self.auv_state[0], self.auv_state[1], radius=3.0):
            print(f"警告: 时间 {self.current_time:.1f}s - AUV与障碍物碰撞！")
            # 碰撞后AUV速度减半并反向（模拟碰撞效果）
            self.auv_state[2] *= -0.5
            self.auv_state[3] *= -0.5
            self.collision_count += 1
    ...
    def _render(self):
        ...
        # 绘制障碍物
        for obstacle in self.sim_map.obstacles:
    ...
    def _draw_status(self):
        """绘制状态信息"""
        # 状态信息背景
        pygame.draw.rect(self.screen, (240, 240, 240), (30, 30, 400, 240))
        ...
        # 碰撞信息
        collision_text = self.font.render(f"碰撞次数: {self.collision_count}", True, (255, 0, 0))
        self.screen.blit(collision_text, (50, 240))
        
        # 意图概率
        intent_text = self.font.render(
            f"意图概率: 正常={self.cas.dbn.intent_probs[0]:.2f}, " + 
            f"碰撞风险={self.cas.dbn.intent_probs[1]:.2f}, " + 
            f"紧急避让={self.cas.dbn.intent_probs[2]:.2f}", 
            True, (0, 0, 0))
        self.screen.blit(intent_text, (50, 280))
    ...
    def _save_results(self):
        ...
        # 保存数据
        np.save(os.path.join(self.save_dir, 'auv_trajectory.npy'), auv_trajectory)
        np.save(os.path.join(self.save_dir, 'rov_trajectory.npy'), rov_trajectory)
        np.save(os.path.join(self.save_dir, 'control_inputs.npy'), control_inputs)
        np.save(os.path.join(self.save_dir, 'rov_controls.npy'), rov_controls)
        np.save(os.path.join(self.save_dir, 'intent_probs.npy'), intent_probs)
        np.save(os.path.join(self.save_dir, 'human_intent_type.npy'), self.human_intent_type)
...
def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='AUV与ROV交互式模拟')
